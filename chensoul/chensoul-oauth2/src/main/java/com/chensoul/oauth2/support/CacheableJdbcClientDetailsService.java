...
import static com.chensoul.oauth2.constants.SecurityConstants.OAUTH_CLIENT_DETAIL;
...
public class CacheableJdbcClientDetailsService extends JdbcClientDetailsService {
    ...
    public ClientDetails loadClientByClientId(final String clientId) throws InvalidClientException {
        ...
        final String clientDetailsValue = (String) this.redisTemplate.opsForHash().get(OAUTH_CLIENT_DETAIL, clientId);
    ...
    public void removeRedisCache(final String clientId) {
        this.redisTemplate.opsForHash().delete(OAUTH_CLIENT_DETAIL, clientId);
    }
    ...
    public void updateRedisCache(final ClientDetails clientDetails) {
        this.redisTemplate.opsForHash().put(OAUTH_CLIENT_DETAIL, clientDetails.getClientId(), JsonUtils.toJson(clientDetails));
    }
    ...
    public void loadAllClientToCache(final List<ClientDetails> clientDetails) {
        if (this.redisTemplate.hasKey(OAUTH_CLIENT_DETAIL)) {
            return;
        }
