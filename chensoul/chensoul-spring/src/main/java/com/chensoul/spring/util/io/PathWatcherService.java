...
public class PathWatcherService implements WatcherService, Runnable, DisposableBean {
        ...
        if (shouldEnableWatchService()) {
            try {
                ...
                while ((key = watchService.take()) != null) {
                    ...
                    val valid = key.reset();
                    if (!valid) {
                        log.info("Directory key is no longer valid. Quitting watcher service");
                    }
                }
    ...
    @Override
    public void close() {
        log.trace("Closing service registry watcher thread");
        IOUtils.closeQuietly(watchService);
        if (this.thread != null) {
            thread.interrupt();
        }
        log.trace("Closed service registry watcher thread");
    }
