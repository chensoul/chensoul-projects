...
public abstract class MethodUtils {
    ...
    public static String getSignature(Method method) {
        ...
        signatureBuilder.append(declaringClassName).append(SymbolConstants.SHARP_CHAR).append(methodName).append(LEFT_PARENTHESIS_CHAR);

        for (int i = 0; i < parameterCount; i++) {
            String parameterTypeName = parameterTypeNames[i];
            signatureBuilder.append(parameterTypeName);
            if (i < parameterCount - 1) {
                signatureBuilder.append(SymbolConstants.COMMA_CHAR);
            }
            parameterTypeNames[i] = null;
        }

        signatureBuilder.append(SymbolConstants.RIGHT_PARENTHESIS_CHAR);
