...
package com.chensoul.openfeign.sentinel.ext;
...
import com.chensoul.openfeign.annotation.FeignRetry;
...
public class AutoFallbackSentinelInvocationHandler implements InvocationHandler {
    ...
    AutoFallbackSentinelInvocationHandler(final Target<?> target, final Map<Method, InvocationHandlerFactory.MethodHandler> dispatch,
                                          final FallbackFactory<?> fallbackFactory) {
    ...
    public Object invoke(final Object proxy, final Method method, final Object[] args) throws Throwable {
        ...
        if (target instanceof Target.HardCodedTarget) {
            final Target.HardCodedTarget<?> hardCodedTarget = (Target.HardCodedTarget) target;
            final MethodMetadata methodMetadata = SentinelContractHolder.METADATA_MAP
                .get(hardCodedTarget.type().getName() + Feign.configKey(hardCodedTarget.type(), method));
            // resource default is HttpMethod:protocol://url
            if (methodMetadata == null) {
                result = methodHandler.invoke(args);
            } else {
                final String resourceName = methodMetadata.template().method().toUpperCase() + ':' + hardCodedTarget.url()
    ...
    public boolean equals(final Object obj) {
        if (obj instanceof SentinelInvocationHandler) {
            final AutoFallbackSentinelInvocationHandler other = (AutoFallbackSentinelInvocationHandler) obj;
    ...
    static Map<Method, Method> toFallbackMethod(final Map<Method, InvocationHandlerFactory.MethodHandler> dispatch) {
        final Map<Method, Method> result = new LinkedHashMap<>();
        for (final Method method : dispatch.keySet()) {
