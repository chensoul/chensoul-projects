...
http://localhost:6543/oauth2/authorize?client_id=messaging-client&redirect_uri=https://www.baidu.com&response_type=code&scope=message.write
...
RESPONSE=$(curl --request POST -s --url http://messaging-client:secret@localhost:6543/oauth2/token \
...
ACCESS_TOKEN=$(curl --request POST -s --url http://messaging-client:secret@localhost:6543/oauth2/token --header 'content-type: multipart/form-data' \
